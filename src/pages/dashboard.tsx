...
export const getServerSideProps: GetServerSideProps<{
  ...
  try {
    const doctors: Array<IDoctors> = await doctorService.search(cookies[TOKEN]);
    return { props: { doctors, error: null } };
  } catch (e) {
    return { props: { error: JSON.stringify(e), doctors: [] } };
  }
};

export default function Home({ doctors }: { doctors: Array<IDoctors> }) {
  return (
    <main className="border-1 w-full h-full bg-gray-50 text-black p-8">
      ...
      <AppointmentContent doctors={doctors} />
    </main>
  );
}
